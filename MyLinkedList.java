...
public class MyLinkedList {
	...
	{ //passed test
		...
		for (ListNode prev = this.head, cur = this.head.next; cur != null;) {
			if (cur.data == o) {
				prev.next = cur.next;
				this.size--;
				cur = cur.next;
	...
	{
		if (index < 0 || index > this.size) throw new IndexOutOfBoundsException("Index Passed in not valid!");
	...
	{
		if (index < 0 || index >= this.size) throw new IndexOutOfBoundsException("Provided index is out of bounds!");
